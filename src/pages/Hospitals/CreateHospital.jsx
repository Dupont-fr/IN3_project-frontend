...
export default function CreateHospital() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.nameHospital.trim()) {
      setError(t('createHospital.error_required'))
      return
    }
    setError('')
    setLoading(true)
    try {
      ...
      setError(err.response?.data?.message || t('createHospital.error_create'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('createHospital.title')}</h1>
        <p className="text-sm text-gray-500 mt-1">{t('createHospital.subtitle')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('createHospital.field_name')} <span className="text-red-500">*</span>
          </label>
          <input
            type="text"
            ...
            placeholder={t('createHospital.field_name_placeholder')}
            required
          />
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
            {t('createHospital.field_services')}
          </label>
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder={t('createHospital.service_placeholder')}
            ...
            >
              {t('createHospital.service_add')}
            </button>
          </div>
          {form.servicesHospital.length > 0 && (
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {loading ? t('createHospital.creating') : t('createHospital.create')}
          </button>
        </div>
      </form>
    </div>
