...
export default function CreateUser() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || t('createUser.error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-lg mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('createUser.title')}</h1>
        <p className="text-sm text-gray-500 mt-1">{t('createUser.subtitle')}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('createUser.field_name')}</label>
          <input
            type="text"
            name="nameUser"
            value={form.nameUser}
            ...
            placeholder={t('createUser.field_name_placeholder')}
            required
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('createUser.field_email')}</label>
          <input
            type="email"
            name="emailUser"
            ...
            onChange={handleChange}
            className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none"
            placeholder={t('createUser.field_email_placeholder')}
            required
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('createUser.field_password')}</label>
          <div className="relative">
            ...
            value={form.passwordUser}
              ...
              placeholder={t('createUser.field_password_placeholder')}
              required
              minLength={8}
            />
            <button
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('createUser.field_role')}</label>
          <select
            name="roleUser"
            value={form.roleUser}
            onChange={handleChange}
        ...
        {(form.roleUser === 'MEDECIN' || form.roleUser === 'ACCUEIL') && (
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              {t('createUser.field_hospital')} <span className="text-red-500">*</span>
            </label>
            <SearchableSelect
              items={hospitals}
              ...
              placeholder={t('createUser.field_hospital_placeholder')}
            />
          </div>
        )}
        ...
        {form.roleUser === 'MEDECIN' && (
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              {t('createUser.field_specialty')} <span className="text-red-500">*</span>
            </label>
            <select
              name="specialtyUser"
              ...
              className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none"
            >
              <option value="">{t('createUser.field_specialty_placeholder')}</option>
              {SPECIALTIES.map((s) => (
