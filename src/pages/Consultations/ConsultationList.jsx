  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        ...
        {canCreate && (
          <Link to="/consultations/create" className="flex items-center justify-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
                    <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_patient')}</th>
                    <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_date')}</th>
                    {!isAccueil && <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_motif')}</th>}
                    <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_doctor')}</th>
                    <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_hospital')}</th>
                    <th className="text-left px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_status')}</th>
                    <th className="text-right px-4 sm:px-6 py-3 text-xs font-medium text-gray-500 uppercase">{t('consultationList.col_actions')}</th>
                  </tr>
                </thead>
                  ...
                  {consultations.map((c) => {
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-4 sm:px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{c.doctorName || '-'}</td>
                        <td className="px-4 sm:px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{c.doctorHospital || '-'}</td>
                        <td className="px-4 sm:px-6 py-4">
                ...
                <div key={c.id} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-medium text-gray-900 dark:text-white truncate">{c.patientName || '-'}</p>
                        <p className="text-xs text-gray-500">{c.date ? new Date(c.date).toLocaleDateString('fr-FR') : '-'}</p>
                      </div>
                    </div>
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-medium rounded-full shrink-0 ${st.class}`}>
                      <StatusIcon className="w-3 h-3" /> {t(st.labelKey)}
                    </span>
                  </div>
                  {!isAccueil && c.motifConsultation && (
                    <p className="text-xs text-gray-500 mb-2 line-clamp-2">{c.motifConsultation}</p>
                  )}
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500">
                    <div className="flex items-center gap-3">
                      {c.doctorName && <span className="font-medium text-gray-700 dark:text-gray-300">{c.doctorName}</span>}
                      {c.doctorHospital && <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{c.doctorHospital}</span>}
                    </div>
